...
def main():
    ...
    # Latency summary: per-α mean and stdev. Latency varies because
    # different routing decisions (corpus vs corpus+web) have different
    # cost profiles, and different α can flip routing on borderline
    # queries.
    print("\n=== Summary (Latency in ms by α) ===")
    lat_summary = df.groupby("alpha")["latency_ms"].agg(["mean", "std", "min", "max"]).round(0)
    lat_summary.columns = ["mean_ms", "std_ms", "min_ms", "max_ms"]
    print(lat_summary.to_string())

    # Latency by α × question_type — useful for spotting whether α
    # changes shift the latency profile asymmetrically across types.
    print("\n=== Mean latency (ms) by α × question_type ===")
    lat_by_type = (
        df.groupby(["alpha", "question_type"])["latency_ms"]
        .mean()
        .unstack()
        .round(0)
    )
    print(lat_by_type.to_string())
