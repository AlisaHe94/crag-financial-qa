...
class CorrectedRAG:
    ...
    def _query_with_decomposition(self, original_question: str,
        ...
        """
        # v3 fix: decomposed sub-questions are corpus-bound (no web
        # fallback). The multimodal decomposition splits a query about
        # ONE entity in ONE corpus filing into table-vs-narrative parts,
        # both of which are supposed to be answerable from corpus.
        # Letting them trigger web fallback (the v2 behavior) introduced
        # off-topic web content (e.g., 2013 Annual Report passages
        # appearing in answers about FY2025) — see ../v2/crag_pipeline.py
        # for the prior unfixed version.
        sub_results: list[dict] = []
        for i, sq in enumerate(sub_questions, 1):
            logger.info(f"=== Sub-question {i}/{len(sub_questions)}: {sq} ===")
            r = self.query(sq, _skip_decomposition=True, _suppress_web=True)
    ...
    def query(
        ...
        #
        # `_suppress_web` is set internally by _query_with_decomposition
        # when running sub-questions. Rationale: the multimodal-decomposition
        # case splits a query about a SINGLE entity in a SINGLE corpus filing
        # into table-vs-narrative sub-questions, both of which are supposed
        # to be answerable from corpus. Letting sub-questions trigger web
        # fallback on AMBIGUOUS routing pulls in irrelevant web content
        # (e.g., a 2013 Annual Report when asked about FY2025) that pollutes
        # the synthesized answer and tanks the LLM-as-judge faithfulness
        # metric. Top-level OOC queries still use web normally.
        ...
        # Effective web-fallback flag for this call: must be both
        # (a) globally enabled (demo control) and (b) not suppressed
        # by a parent decomposition call.
        web_enabled = self.web_fallback_enabled and not _suppress_web
        ...
        if query_type == "out_of_corpus" and web_enabled:
        ...
        elif query_type == "out_of_corpus" and not web_enabled:
        ...
        # _suppress_web=True (set by decomposition) also disables web for
        # sub-questions to prevent off-topic web pollution.
        use_web = trigger_web and not strong_corpus_match and web_enabled
        ...
        if (
            ...
            try:
                if not self._answer_addresses_all_parts(question, answer):
                    ...
                    if still_incomplete and max_cosine < 0.65 and web_enabled:
                        logger.info(
