...
if run and question.strip():
    ...
    with col_crag:
        ...
        # Sources used (if any) — small footer chip. Citation-grounded
        # filter: top-K retrieval often returns semantically-similar
        # chunks from related filings (e.g., an Amazon 10-Q chunk
        # surfacing on an Apple net-sales query because both contain
        # phrases like "total net sales" and "fiscal year 2025").
        # Showing every retrieved source as a chip makes the list look
        # noisy and not actually grounded. We filter to only those
        # filings whose readable name appears in the generated answer
        # text — those are the sources the LLM actually cited. If
        # nothing matches (e.g., the answer doesn't name any source),
        # we fall back to showing all retrieved sources for
        # transparency.
        sources = crag_result.get("sources_used", [])
        if sources:
            answer_text = clean_answer(crag_result.get("answer", "")).lower()
            seen: set[str] = set()
            ...
            for s in sources:
                ...
                if not pretty or pretty in seen:
                    continue
                seen.add(pretty)
                all_sources.append(pretty)
                # Match the full readable name OR the company+filing-type
                # core (e.g. "Apple 10-K" without the year).
                core = pretty.split(" (")[0].lower()
                if pretty.lower() in answer_text or core in answer_text:
                    cited_sources.append(pretty)

            display_sources = cited_sources if cited_sources else all_sources
            src_chips = " ".join(
