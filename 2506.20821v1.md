MultiFinRAG: An Optimized Multimodal Retrieval-Augmented
...
egy then dynamically escalates from text-only to text+table+image
...
driving up API costs and making end-to-end processing infeasible.

1

Mixed formats: Structured tables and visual figures lose their
...
• Employ static top-𝑘 retrieval, which can return redundant or
...
analysis
locates charts and diagrams within
...
3.3.1
𝐹𝑖 is segmented into three sets of retrievable chunks:

System Overview. Figure 1 illustrates our pipeline. Each PDF
...
are semantically coherent text passages, and 𝐶table
where 𝐶text
𝑖
𝐶image
are table and figure regions converted via a multimodal
LLM. All chunks are embedded and stored in an approximate FAISS
index. A query 𝑄 triggers a tiered retrieval (text-only then text
...
(2) Sliding windows: with window size 𝑤 and overlap 𝑜, form
...
(3) Embedding & breakpoints: embed each sentence 𝑒 𝑗 = 𝐸 (𝑠 𝑗 ),
...
and mark any 𝑑 𝑗 above the 95th percentile of {𝑑 𝑗 } as a split
point.

(4) Chunk formation: split each block 𝐵𝑖 at its breakpoints into
...
Require: PDF docs F , batch size 𝐵, embedder 𝐸, empty indexes
...
Render pages → 𝑃𝑇 , 𝑃𝑉
Detect tables 𝑇 ← detect_tables(𝑃𝑇 )
Detect figures 𝑉 ← detect_figures(𝑃𝑉 )
Text extraction & semantic chunking:

𝑋 ← extract_text(𝐹 )
𝑆 ← segment_sentences(𝑋 )
𝐶 ← semantic_chunk_merge(𝑆)

for all 𝑐 ∈ 𝐶 do

𝑒 ← 𝐸 (𝑐); normalize(𝑒)
Itext.insert(𝑒, { content : 𝑐})

end for
Batch-parse tables:

Partition 𝑇 into ⌈|𝑇 |/𝐵⌉ batches

for all batch 𝑡𝑏 do

{(𝑑𝑒𝑠𝑐𝑖, 𝑗𝑠𝑜𝑛𝑖 )} ← batch_parse_one_batch(𝑡𝑏 )
for all (𝑑𝑒𝑠𝑐𝑖, 𝑗𝑠𝑜𝑛𝑖 ) do

𝑒 ← 𝐸 (𝑑𝑒𝑠𝑐𝑖 ); normalize(𝑒)
Itable.insert(cid:0)𝑒, { summary : 𝑑𝑒𝑠𝑐𝑖, json : 𝑗𝑠𝑜𝑛𝑖 }(cid:1)
...
Partition 𝑉 into ⌈|𝑉 |/𝐵⌉ batches

for all batch 𝑣𝑏 do

{𝑠𝑢𝑚 𝑗 } ← batch_summarize(𝑣𝑏 )
for all 𝑠𝑢𝑚 𝑗 do

𝑒 ← 𝐸 (𝑠𝑢𝑚 𝑗 ); normalize(𝑒)
Iimage.insert(𝑒, {summary : 𝑠𝑢𝑚 𝑗 })
...
• Detected table regions 𝑇 are first cropped with padding and saved
as individual images. We then partition 𝑇 into ⌈|𝑇 |/𝐵⌉ batches
of size at most 𝐵. For each batch 𝑡𝑏 (lines 11–17):

• We construct a single multimodal prompt listing the exact file-
names in 𝑡𝑏 and invoke batch_parse_one_batch, which returns
(desc𝑖, json𝑖 ) for each file.

• Each textual description desc𝑖 is embedded using the model 𝐸 (·)
and inserted into the table index Itable along with its parsed JSON
representation.
...
• The returned summaries {sum𝑗 } are embedded, normalized, and
...
figures.
By batching up to 𝐵 items per prompt, we amortize the LLM’s
...
3.3.4 Tiered Retrieval & Decision Function. Let 𝑛, 𝑚, and 𝑝 denote
...
(cid:12)
(cid:12) cosine(cid:0)𝑒𝑄, 𝐸 (𝑐)(cid:1) ≥ 𝜃text

(cid:9).

If |T | ≥ 𝑛, issue a single LLM call with T as context and
terminate.
(2) Table fallback.

Ttbl = Top𝑚

(cid:8) 𝑐 ∈ 𝐶table (cid:12)

(cid:12) cosine(cid:0)𝑒𝑄, 𝐸 (𝑐)(cid:1) ≥ 𝜃table
...
(cid:8) 𝑐 ∈ 𝐶image (cid:12)

(cid:12) cosine(cid:0)𝑒𝑄, 𝐸 (𝑐)(cid:1) ≥ 𝜃image

(cid:9).

(4) Combined prompt. Concatenate any non-empty sets among
T , Ttbl, and Timg (including each table’s JSON + summary), then
...
racy:
(1) Text threshold sweep: Vary 𝜃text from 0.55 to 0.85 in
steps of 0.05; at each value, retrieve all text chunks with
cos(𝐸 (𝑄), 𝐸 (𝑐)) ≥ 𝜃text, feed the top-𝑘 to the LLM, and record
answer accuracy.

(2) Table & image threshold sweep: Keeping 𝜃text fixed, vary
...
accuracy. Detailed information-retrieval statistics (precision, recall,

5

etc.) for the retrieval components were collected during develop-
...
Number of
Baseline
Questions with Llama with Gemma

Baseline MultiFinRAG MultiFinRAG ChatGPT-4o
with Llama with Gemma
(Free Tier)

1. Text-based
2. Image-based
3. Table-based
4. Text+Image/Table-based
Total
...
Image-based questions. This type of questions require ana-
4.1.2
...
Another example from Figure 6 is: "What was Sasan K.
...
Image-based Questions. The accuracy difference for image-
4.3.2
...
Image-based Questions. MultiFinRAG with Gemma 3
4.4.2
...
summaries of longitudinal changes.

8

Chinmay Gondhalekar, Urjitkumar Patel, and Fang-Chun Yeh

• Robustness to Noise & Error Correction: PDF parsing and
...
Strong LLM Is All You Need. arXiv:2406.18064 [cs.CL] https://arxiv.org/abs/
2406.18064
...
arXiv:2310.02567 [cs.CV]

VQA Evaluation Using Large Language Models.
...
Jon Saad-Falcon, Joe Barrow, Alexa Siu, Ani Nenkova, David Seunghyun Yoon,
...
John Smith, Jane Doe, and Emily Johnson. 2024. Financial Report Chunking
